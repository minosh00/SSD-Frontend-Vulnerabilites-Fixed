...
const AllEmployee = () => {
  ...
  useEffect(async () => {
    try {
      const data = await (await axios.get(`${StartUrl}api/employees/`)).data;
      setusers(data);
      setloading(false);
    } catch (error) {
      console.log(error);
  ...
  const removeEmployee = (id) => {
    axios.delete(`${StartUrl}api/employees/${id}`).then((res) => {
          ...
          {Employee &&
            Employee.filter((users) => {
              if (serachItem == "") {
                ...
                users.fname.toLowerCase().includes(serachItem.toLowerCase())
              ) {
                return users;
              }
            }).map((user) => {
